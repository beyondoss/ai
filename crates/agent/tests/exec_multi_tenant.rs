...
/// A child's registry, built the way `subagent::build_child_registry` builds it — from the parent's
/// `ChildToolConfig`. If the parent hands down `None`, the child lands on the host, and the model can
/// step outside the sandbox just by delegating.
fn child_registry_from(parent_cell: &ExecCell) -> agent_core::ToolRegistry {
    default_registry_with_config(&ToolConfig {
        fs_backend: Some(parent_cell.backend()),
        command_runner: Some(parent_cell.runner()),
        ..ToolConfig::new()
    })
}

#[tokio::test]
async fn a_subagent_acts_on_its_parents_machine_not_the_host() {
    // The escape this closes: before the fix, both `serve` and `run` passed `fs_backend: None` and
    // `command_runner: None` into `ChildToolConfig`, so a sandboxed parent spawned children whose
    // `read`, `write` and `bash` ran on the host. `subagent`'s own module doc calls a child without
    // the parent's constraints "the sandbox escape"; this asserts it is not one.
    let tenant = tenant_dir("tenant-a", "SECRET-AAAA");
    let cell = ExecCell::new();
    cell.set(Some(tenant_target(tenant.path()).await));

    let child = child_registry_from(&cell);

    // A relative path resolves on whichever machine the child is actually using.
    let read = call(&child, "read", json!({ "path": "secret.txt" })).await;
    assert!(
        read.contains("SECRET-AAAA"),
        "the child's `read` did not reach the parent's sandbox: {read}"
    );

    let bash = call(&child, "bash", json!({ "command": "cat secret.txt" })).await;
    assert!(
        bash.contains("SECRET-AAAA"),
        "the child's `bash` ran somewhere else — this is the escape: {bash}"
    );

    // And a child write lands in the sandbox, not on the host.
    call(
        &child,
        "write",
        json!({ "path": "by-child.txt", "content": "child-was-here\n" }),
    )
    .await;
    assert!(
        tenant.path().join("by-child.txt").exists(),
        "the child's `write` did not land in the parent's sandbox"
    );
}

#[tokio::test]
async fn a_subagent_follows_the_parent_when_the_parent_is_repointed() {
    // Children are built per spawn from the parent's config. Because that config carries the *cell*
    // rather than a snapshot, a child spawned after a re-point uses the new target — a child holding
    // a stale snapshot would be operating in the previous tenant's sandbox.
    let a = tenant_dir("tenant-a", "SECRET-AAAA");
    let b = tenant_dir("tenant-b", "SECRET-BBBB");
    let cell = ExecCell::new();
    ...
    cell.set(Some(tenant_target(a.path()).await));
    let first = call(
        &child_registry_from(&cell),
        "read",
        json!({ "path": "secret.txt" }),
    )
    .await;
    assert!(first.contains("SECRET-AAAA"), "{first}");
    ...
    .await;
    assert!(second.contains("SECRET-BBBB"), "{second}");
    assert!(
        !second.contains("SECRET-AAAA"),
        "a child spawned after the re-point still reached the previous tenant: {second}"
    );

    // Even a registry built *before* the re-point follows it, because it holds the cell.
    let stale = child_registry_from(&cell);
    cell.set(Some(tenant_target(a.path()).await));
    let after = call(&stale, "read", json!({ "path": "secret.txt" })).await;
    assert!(
        after.contains("SECRET-AAAA") && !after.contains("SECRET-BBBB"),
        "a child registry built earlier did not follow the parent: {after}"
    );
}
