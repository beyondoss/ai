...
- **Trust** ([`trust_store`](src/trust_store.rs)) — a tri-state, ancestor-inheriting allowlist
  ...
  **Not** gated by trust: `AGENTS.md`/`CLAUDE.md` project-instruction files
  ...
  `.claude/skills`, no `.claude/prompts`, no `.agents/skills` anywhere between `cwd` and the enclosing
...
- **Persisted settings** ([`settings`](src/settings.rs)) — pi's own `SettingsManager`, scoped way down:
  ...
  `block_images`/`image_auto_resize`/`thinking_budget_overrides` —
  ...
  --default-reasoning-effort/--block-images/--image-auto-resize/--thinking-budget EFFORT=TOKENS <val>]
  ...
  --clear-thinking-budget EFFORT]` — the same
  managed-out-of-band
