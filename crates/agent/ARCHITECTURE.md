...
- **Persisted settings** ([`settings`](src/settings.rs)) — pi's own `SettingsManager`, scoped way down:
  ...
  explicit list falls back to the stored one *in full*; a non-empty explicit list wins outright with no
  ...
  tools" section above), the identical tri-state convention just inverted (defaults *on*, where
  `block_images` defaults *off*); `thinking_budget_overrides` (Task #36 pi-parity feature) is a per-effort-level
  ...
  `Persistence::open` checks `--session-dir` *before* `--session-file`, so the stored
  `default_session_dir` fallback only applies when `--session-file` is *also* absent — otherwise a stored
  ...
  **Project-level settings tier** (Round 3, pi-parity feature: `settings::effective_settings_for_cwd`,
  `Settings::merge_over`) — `SettingsStore` above is the *global* tier only (one per machine); a project
  ...
  list fields (`default_models_list`/`default_skill_paths`/`default_prompt_template_paths`) *replace*
  ...
  concat/append. Gated on `cwd` being trusted per the *persisted* `trust_store::TrustStore` allowlist
  ...
  *persisted* `agent trust <path>` grant is trusted enough for that. An untrusted project's
  ...
  above), so a project with *only* a settings.json (no `SYSTEM.md`/skills/prompts) still requires trust
...
- **System-prompt assembly** ([`resources`](src/resources.rs)) — split into a **static** half (base
  ...
  **deliberate divergence, awareness-only: a blank/whitespace-only `SYSTEM.md`/`APPEND_SYSTEM.md` or
  ...
  `loadContextFileFromDir` (the prior version committed to a name the moment it was *seen*, so either
  ...
  **Deliberate divergence, awareness-only: within one root, a loose `name.md` file and a `name/SKILL.md`
  ...
  `fold_skills_later_wins` keeps whichever was folded *later* — so the loose file always wins, by
  ...
  it saw *first* in that unspecified order — an accidental byproduct of `readdirSync`, not a rule.
  ...
  **Deliberate divergence, awareness-only: the final listing order — both `<available_skills>` and
  ...
  it), at the cost of no longer reflecting *where* a skill/template came from just by its position in the
...
- **Branch-local model/thinking-level** — `set_model`/`cycle_model`/`set_reasoning_effort`/
  ...
  equivalent field on `SessionMeta` to fall back to, so it falls back to the *process's own starting
  level* (`--reasoning-effort`, or `Off`) instead — the same "no change recorded yet" logic, sourced
  ...
  **A fresh fork's own header carries the title in effect at the fork point, not `SessionMeta.title`'s
  ...
  which — unlike `meta.model` — isn't a frozen creation-time value but the *live* session's own
...
- **Reset to root** — `switch_branch`'s `before: bool` (mirroring `fork`'s identical flag) switches to
  ...
  for a `before`-resolved switch queries the *resolved* target (`Persistence::model_and_level_at_opt`,
  ...
  the resolved parent, since an anchored change takes effect for what comes *after* that point, not the
  point itself.
- **Expanded `serve` surface** — beyond `prompt`/`get_state`/`get_messages`/`new_session`: `abort`,
  ...
  *unique* prefix of one — `SessionRepo::find_path` tries an exact match first, then a prefix scan; an
  ...
  crate's own arbitrary-session/arbitrary-point *preview* feature instead, breaking a pi-compatible
...
- **Whole-run auto-retry** — once `agent_core`'s own mid-stream retry (inside one model turn) is
  ...
  matches pi's own broader net, which is pi's *only* layer for a plain HTTP-status failure since SDK
  retry is disabled there) is automatically re-invoked against the *same* session — resuming from
  ...
  a hiccup" from silence. The backoff sleep *is* raced — against `abort`/`abort_retry`/shutdown, via a
  ...
  A cancelled retry surfaces the *real* error that triggered it, not a synthetic `Error::Cancelled` —
...
- **Multimodal** — `prompt` accepts `images: [{media_type, data}]` (base64), built into a multimodal
  ...
  note states both dimension pairs *and* the coordinate-scale multiplier (`orig_w / new_w` — the resize
...
failed partway through left *no session record at all* of the round-trips that had already happened —
...
(the RunningTurn→Ready transition row above) reflects only the *final* post-attempt persist's own
...
directly: this agent's tool calls have *real* side effects (a `bash` command, an `edit`/`write`), and a
...
deferring creation until an assistant reply exists would mean the *first* turn's tool calls have nowhere
...
into the *same* `CancellationToken` `agent_core::Agent::run[_events]_cancellable` already drops tool
...
Task #41 (pi-parity fix): `ShutdownSignal::wait` now returns *which* signal actually fired
...
- **`read`** numbers every line (`{lineno:>6}\t{line}`), 1-based `offset`, default `limit` 2000 — also
  the hard ceiling for an *explicit* `limit`: a caller asking for `limit: 5000` still gets capped at
  ...
  to image/png.]"` hint — pi's `image-process.ts` appends the same note, so the transcript states *why*
...
- **`write`** creates parent directories (`create_dir_all`), then writes **atomically** (sibling temp
  ...
  otherwise never see this: `rename(2)` only consults the *containing directory's* write permission,
...
`*.md` still finds `README.MD`); a pattern with *any* uppercase letter matches exactly as typed —
...
`grep` additionally clips any single match line to 500 *characters* (`clip()` in `grep.rs`) so one
...
marker; the byte cap is checked *before* any count/line marker is appended and wins outright when any
...
whatever *did* happen — `match limit N reached; …` and/or, when any individual line was clipped, `some
...
non-zero and report the actual error text the moment they can't read *any* path in the tree — a chmod
`000` directory, another user's home, a locked-down mount silently reading as "nothing here" would
otherwise mislead the model into concluding the pattern/file doesn't exist. When real matches *were*
...
- **`bash`**: runs through a **resolved real `bash`** (`/bin/bash`, else `bash` on `$PATH`, else `sh` —
  ...
  suppressed until the *next* chunk, which here never comes).
...
| Ready             | `{"type":"new_session"}`, on-disk reset/create fails   | Ready       | —                      | `response{success:false, error}` — the *previous* session stays active (nothing is swapped for an empty in-memory session that was never actually persisted, which would otherwise desync `SessionStore`'s persisted-message count from what the caller believes is live, silently dropping the next turn's `append_new`) |
...
| Ready             | `{"type":"set_model"}` / `"cycle_model"` / `"set_reasoning_effort"` / `"cycle_thinking_level"`, recording the change fails to persist | Ready | — | `response{success:false, error}`; the live model/thinking-level is *not* switched either — a failed persist never leaves live state ahead of what's durably recorded |
...
this reason, but that only covers *our own* `tracing::warn!`/`info!` calls, not a direct `println!`/
`print!`. pi's equivalent (`output-guard.ts`'s `takeOverStdout`) closes that gap at runtime: it
monkey-patches `process.stdout.write` globally, transparently redirecting *any* code's stray write
...
arm and `false` in the idle one — architecturally, those are the *only* two places `get_state` can ever
run from, so which arm answered already says everything. `is_compacting` is different: `Agent::compact`
runs *inside* the busy loop's own event stream, so its state has to be tracked live. It's set on the new
...
`session_id`/`session_file` *and* the whole `user_messages`/`assistant_messages`/`tool_calls`/
...
in-flight turn — present as `null` rather than silently absent, so a client sees the same field *set*
...
*model* — the model-facing text is the same bare `"Successfully replaced N block(s) in {path}."`
...
tool that streams incremental state). And where a diff actually gets *rendered* today —
`export.rs`'s `render_edit_call` — it already reconstructs the old/new pair independently from the
tool *call's `input`* via `parse_edits`, the same parser the tool itself runs, not from anything
...
`console.warn`), but it has zero call sites anywhere in pi's own current source; the only *actual*
...
- **BYO** — any other string, forwarded to the upstream provider *unchanged*
...
- **OAuth subscription** (`crate::oauth`/`crate::auth_store`) — a Claude Pro/Max, GitHub Copilot, or
  ...
  gateway needed *zero* changes to support it. The insight that made this the right design: a
...
learning *how* a token was obtained: `GatewayClient` resolves a fresh `Credential` immediately before
...
`agent_core`, never the reverse. `agent_core` still never *holds* a provider key of its own; it only
...
- **Project trust gates *some* instruction sources, not tool execution.** An untrusted working
  ...
  `project_trusted` as a *required* parameter that adds or omits *only* the project-local roots, and
...
| `tests/oauth_e2e.rs`, `tests/serve_oauth_model_switch.rs` | A seeded `~/.claude/auth.json` (no `--key`/`AI_AGENT_KEY`) driving `run`/`serve` against a mock model server: `oauth_e2e` proves a stored Anthropic credential alone is enough to authenticate a `run` turn, carrying its OAuth identity headers, and that no credential at all is a clean error naming `agent login`; `serve_oauth_model_switch` proves `set_model` across two *different* stored OAuth logins (Anthropic → OpenAI Codex) re-derives the gateway credential/routing for the new provider on the very next turn rather than reusing whichever client was resolved at startup — regression coverage for `gateway_credential::resolve_gateway_credential` being re-run on every model switch, not just once at process start |
...
*why* this needed no gateway changes at all and the identity-header tradeoff; this section is *what's
built* and *where*.
...
new piece of substance: double-checked-locked refresh (the lock is held across the *entire* refresh,
...
id; Copilot: *not* inferred by model prefix at all, since its model set is dynamic — matched only
