...
| `DEFAULT_MAX_STEPS` / `Agent::with_max_steps`   | 24                   | Loop-iteration ceiling; once `session.steps` reaches it, the _next_ iteration returns `Error::MaxSteps` before sending a request — a runaway-tool-call backstop                                                                                                            |
