//! MCP elicitation e2e:
//! - classic nested `elicitation/create` mid-`tools/call` (`ask_user`)
//! - SEP-2322 MRTR `input_required` elicitation under protocol `2026-07-28` (`ask_user_mrtr`)
//!
//! In both cases serve answers via `elicit` and the tool completes with the accepted content.
...
/// SEP-2322: fixture returns `input_required` with an elicitation input request; rmcp's
/// `call_tool` fulfills it via our handler → serve UI, then retries to completion.
#[test]
fn serve_answers_mcp_mrtr_input_required_elicitation() {
    let dir = tempfile::tempdir().unwrap();
    let home = dir.path().join("home");
    std::fs::create_dir_all(&home).unwrap();
    write_global_settings(
    ...
    let session_file = dir.path().join("s.jsonl").to_string_lossy().into_owned();
    let (base, _bodies) = spawn_model_server(vec![
        turn_tool_use("toolu_m", "mcp__ask__ask_user_mrtr", &json!({}).to_string()),
        turn_text("done"),
    ]);
    ...
    writeln!(
        stdin,
        "{}",
        json!({ "type": "prompt", "message": "ask via mrtr" })
    )
    .unwrap();
    stdin.flush().unwrap();
    ...
    while std::time::Instant::now() < deadline {
        ...
        if frame["type"] == "elicitation_request" {
            ...
            assert!(
                msg.contains("MRTR"),
                "expected MRTR elicitation message, got {msg:?}"
            );
            break;
        }
    }
    let elicit_id = elicit_id.expect("timed out waiting for MRTR elicitation_request");
    ...
    let tool_text: String = frames
        ...
        .join("\n");
    assert!(
        tool_text.contains("mrtr-hello-Ferris"),
        "MRTR elicitation content must reach the tool result; got {tool_text:?} frames={frames:?}"
    );
}
